...
function scanQR(url, my){
    ...
    var i = setInterval(function(){
        counter+=1; 
        if (counter==5){
            document.getElementsByClassName("OwnerPageCover__in")[0].outerHTML=HTMLparts[0]+''+HTMLparts[3];
            clearInterval(i)
        }
        if (link!='')
        { /*/console.log(link), /*/
        ...
        setTimeout(window.onload = function(){
            try{    
                if (document.getElementsByClassName("OwnerPageCover__in")[0].outerHTML.length!=38){
                    getURL(document.getElementsByClassName("OwnerPageCover__in")[0].outerHTML, document.getElementsByClassName("OwnerPageCover__in")[0].outerHTML.length>300)
                } else {
